#ifndef BRUSSELATOR1DIMEXPROBLEM_DEFINED__
#define BRUSSELATOR1DIMEXPROBLEM_DEFINED__
...
class Brusselator1DIMEXProblem : public Problem {
public:
	int n = 0;
	double dx = 0.0;
	int u_start = 0;
	...
	Brusselator1DIMEXProblem(int n_) {
		name = "Brusselator1DIMEX";
		n = n_;
	...
	double d(double t) {
		return 0.00006 + 0.00005*cos(M_PI*t);
	}
	...
	void full_rhs_custom(double t, vec* y, vec* f) {
		...
		for(int i=1; i<n-1; i++) {
			...
			(*f)(u_start+i) = d(t)*(1.0*unm1 -2.0*un + 1.0*unp1)/(dx*dx) + s(t)*(-1.0*unm1 + 1.0*unp1)/dx + r(t)*(a - (wn+1.0)*un + un*un*vn);
			(*f)(v_start+i) = d(t)*(1.0*vnm1 -2.0*vn + 1.0*vnp1)/(dx*dx) + s(t)*(-1.0*vnm1 + 1.0*vnp1)/dx + r(t)*(un*wn - un*un*vn);
			(*f)(w_start+i) = d(t)*(1.0*wnm1 -2.0*wn + 1.0*wnp1)/(dx*dx) + s(t)*(-1.0*wnm1 + 1.0*wnp1)/dx + r(t)*((b - wn)/epsilon - un*wn);
		}
	}
