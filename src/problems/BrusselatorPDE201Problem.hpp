#ifndef BRUSSELATORPDE201PROBLEM_DEFINED__
#define BRUSSELATORPDE201PROBLEM_DEFINED__
...
class BrusselatorPDE201Problem : public Problem {
public:
	int n = 0;
	double dx = 0.0;
	int u_start = 0;
	...
	BrusselatorPDE201Problem() {
		name = "BrusselatorPDE201";
		n = 201;
	...
	void full_rhsjacobian_custom(double t, vec* y, mat* j) {
		...
		for(int i=1; i<n-1; i++) {
			...
			(*j)(u_start+i,u_start+i) = alpha_u*(-2.0)/(dx*dx) + ((*y)(w_start+i)+1.0) + 2.0*(*y)(u_start+i)*(*y)(v_start+i);
	...
	void fast_rhsjacobian_custom(double t, vec* y, mat* j) {
		...
		for(int i=1; i<n-1; i++) {
			(*j)(u_start+i,u_start+i) = ((*y)(w_start+i)+1.0) + 2.0*(*y)(u_start+i)*(*y)(v_start+i);
			(*j)(u_start+i,v_start+i) = (*y)(u_start+i)*(*y)(u_start+i);
			(*j)(u_start+i,w_start+i) = (*y)(u_start+i);
