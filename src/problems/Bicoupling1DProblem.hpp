#ifndef BICOUPLING1DPROBLEM_DEFINED__
#define BICOUPLING1DPROBLEM_DEFINED__
...
class Bicoupling1DProblem : public Problem {
public:
	int n = 0;
	double dx = 0.0;
	int u_start = 0;
	...
	void full_rhs_custom(double t, vec* y, vec* f) {
		...
		for(int i=1; i<n-1; i++) {
			...
			(*f)(u_start+i) = d(t)*(1.0*unm1 -2.0*un + 1.0*unp1)/(dx*dx) + r(t)*(w*vn - wn - p*t);
			(*f)(v_start+i) = d(t)*(1.0*vnm1 -2.0*vn + 1.0*vnp1)/(dx*dx) + r(t)*(-w*un);
			(*f)(w_start+i) = d(t)*(1.0*wnm1 -2.0*wn + 1.0*wnp1)/(dx*dx) + r(t)*(-l*wn - l*p*t - p*std::pow(un - a*wn/(a*l + b*w) - a*p*t/(a*l + b*w), 2.0) - p*std::pow(vn - b*wn/(a*l + b*w) - b*p*t/(a*l + b*w), 2.0));
		}
	}
