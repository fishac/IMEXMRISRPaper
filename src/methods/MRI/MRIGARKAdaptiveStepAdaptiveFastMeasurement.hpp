#ifndef MRIGARKADAPTIVESTEPADAPTIVEFASTMEASUREMENT_DEFINED__
#define MRIGARKADAPTIVESTEPADAPTIVEFASTMEASUREMENT_DEFINED__
...
#include "AdaptiveDIRKMethodFastMeasurement.hpp"
...
class MRIGARKAdaptiveStepAdaptiveFastMeasurement : public MRIGARKAdaptiveStep {
public:
	Problem* problem;
	...
	AdaptiveDIRKMethodFastMeasurement dirk;
	...
	MRIGARKAdaptiveStepAdaptiveFastMeasurement(MRICoefficients* coeffs_, SingleRateMethodCoefficients* inner_coeffs_, Problem* problem_, int problem_dimension_, WeightedErrorNorm* err_norm_, Controller* inner_controller_) :
	inner_rhs_funcs(coeffs_, problem_, problem_dimension_),
	implicit_solve_residual(&(MRIGARKAdaptiveStepAdaptiveFastMeasurement::inner_rhs_funcs)),
	explicit_solve_problem(&(MRIGARKAdaptiveStepAdaptiveFastMeasurement::inner_rhs_funcs)),
	dirk(inner_coeffs_, &(MRIGARKAdaptiveStepAdaptiveFastMeasurement::explicit_solve_problem), problem_dimension_, inner_controller_),
	newton_solver(&(MRIGARKAdaptiveStepAdaptiveFastMeasurement::implicit_solve_residual), 20, 0.1, problem_dimension_, err_norm_)
	{
		problem = problem_;
		coeffs = coeffs_;
	...
	void step_solution(double t, double H, double tolf, vec* y_0, const char* measurement_type, MRIGARKAdaptiveStepReturnValue* ret) {
		prepare_solve();
				...
				if (status == 0) {
					...
					if (coeffs->method_type == 0) {
						...
						if (delta_c == 0) {
							...
							explicit_solve(tolf,stage_index,interval_start,interval_end);
						}
					} else if (coeffs->method_type == 1) {
						explicit_solve(tolf,stage_index,interval_start,interval_end);
					}
	...
	void explicit_solve(double tolf, int stage_index, double interval_start, double interval_end) {
		struct AdaptiveDIRKMethodFastMeasurementReturnValue ret;
		vec output_tspan = { interval_end };
		double h_0 = (interval_end-interval_start);
		//printf("Stage: %d, solving with h_0: %.16f, tolf_0: %.16f, over (%.16f,%.16f)\n",stage_index,h_0,tolf,interval_start,interval_end);
		dirk.solve(interval_start, h_0, tolf, &v_0, &output_tspan, &ret);
		v_H = ret.y;
		errs(stage_index) = ret.err_estimate;
		inner_status = ret.status;
		total_microtimesteps += ret.total_steps;
		total_successful_microtimesteps += ret.total_successful_steps;
