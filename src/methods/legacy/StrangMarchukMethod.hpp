#ifndef STRANGMARCHUKMETHOD_DEFINED__
#define STRANGMARCHUKMETHOD_DEFINED__

#include "StrangMarchukExplicitSolveProblem.hpp"
#include "StrangMarchukImplicitSolveResidual.hpp"
...
public:
	...
	Problem* problem;
	StrangMarchukExplicitSolveProblem explicit_solve_problem;
	StrangMarchukImplicitSolveResidual implicit_solve_residual;
	NewtonSolver newton_solver;
	struct NewtonSolverReturnValue newton_ret;
	FixedDIRKMethod dirk;
	WeightedErrorNorm* err_norm;

	StrangMarchukMethod(SingleRateMethodCoefficients* inner_coeffs_, Problem* problem_, int problem_dimension_, WeightedErrorNorm* err_norm_) : 
	implicit_solve_residual(problem_),
	explicit_solve_problem(problem_),
	dirk(inner_coeffs_, &(StrangMarchukMethod::explicit_solve_problem), problem_dimension_, err_norm_, false),
	newton_solver(&(StrangMarchukMethod::implicit_solve_residual), 20, 1.0, problem_dimension_, err_norm_) 
	{
		problem = problem_;
	...
	void step_solution(double t, double H, int M, vec* y_n, vec* y_np1) {
		...
		f_temp2 = v_0+H/2.0*f_temp;
		problem->explicit_rhs(t+H/2.0, &f_temp2, &f_temp3);
		v_H = v_0 + H/4.0*f_temp + H/4.0*f_temp3;
		
		v_0 = v_H;
		problem->implicit_rhs(t, &v_0, &f_temp);
		implicit_solve_residual.set_explicit_data(&f_temp);
		newton_solver.solve(t+H/2.0, &v_0, &newton_ret);
		...
		v_0 = v_H;
		problem->implicit_rhs(t+H/2.0, &v_0, &f_temp);
		implicit_solve_residual.set_explicit_data(&f_temp);
		...
		v_0 = v_H;
		problem->explicit_rhs(t+H/2.0, &v_0, &f_temp);
		f_temp2 = v_0+H/2.0*f_temp;
		problem->explicit_rhs(t+H, &f_temp2, &f_temp3);
		v_H = v_0 + H/4.0*f_temp + H/4.0*f_temp3;
	...
	void prepare_solve(double H_, vec* output_tspan) {
		...
		f_temp3 = vec(problem_dimension, fill::zeros);
